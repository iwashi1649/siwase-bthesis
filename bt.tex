...
\begin{equation}
  ...
  \end{split}
\end{equation}
ここで，$x_{\mathrm{hypo}}$は仮説のビット列から出力される塩基を表し，$i$は仮説が参照する
$\bm{P}_{\mathrm{CTC}}$の行番号を表す．また，$x_{\mathrm{max}}(i)$は$\bm{P}_{\mathrm{CTC}}$の$i$行目において
...
\begin{enumerate}
  \item $n=l_{\mathrm{p}} + l_{\mathrm{seg}}$とする．ただし$l_{\mathrm{p}}$はプライマー長，$l_{\mathrm{seg}}$はセグメント長を表す．
  \item $n-30<k<n+30$を満たす整数$k$対して$S_{\mathrm{match}}(k)$を以下の式で計算し，
  $S_{\mathrm{match}}(k)$を最小とする$k$をセパレータの位置として選択し，これを$k'$とする．
  \begin{align}
    S_{\mathrm{match}}(k) = \sum_{j=0}^{l_{\mathrm{sep}}-1} \bm{P}_{\mathrm{CTC}}(k+j, S_j) \label{eq:separator_detection}
  \end{align}
  ただし$S_j$はセパレータ配列の$j$番目の塩基を表し，$l_{\mathrm{sep}}$はセパレータ配列の長さを表す．
  \item $n=k' + l_{\mathrm{sep}}+l_{\mathrm{seg}}$とする．

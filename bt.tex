...
SBS法は大量のDNA断片を並列に高精度に解析できる利点があるが，
一度に解析できるDNAの塩基長が数百bp程度と短いことや，解析速度が遅いという欠点がある\cite{SBS}．
一方，ロングリードシーケンサーの一つであるナノポアシーケンサーは
一度に数十万bp程度の長い塩基長のDNAを高速に解析できるため，
DNAストレージの読み出し用のシーケンサーとしてより実用的であると考えられる\cite{Amarasinghe2020}．
...
置換エラーだけでなく挿入・消失エラーに対応した誤り訂正符号の検討が重要である．
...
しかし，先行研究はDNAストレージの読み出しにショートリードシーケンサーを用いることを前提としており，
...
\begin{enumerate}
  ...
  \begin{itemize}
    \item 置換エラー(substitution): 読み出された塩基が本来の塩基とは異なる誤り
    \item 挿入エラー(insertion): 本来の塩基配列に存在しない塩基が読み出される誤り
    \item 消失エラー(deletion): 本来の塩基配列に存在する塩基が読み出されない誤り
  \end{itemize}
...
$I_i$はビット位置のインデックス，$V_i$は直前の12ビットの値である．
...
報酬$P_{\mathrm{ok}}$，
置換ペナルティ$P_{\mathrm{sub}}$，
挿入ペナルティ$P_{\mathrm{ins}}$，
消去ペナルティ$P_{\mathrm{del}}$
を用いて
以下のように計算される．
\begin{itemize}
  ...
  \begin{align}
    \Delta P = \begin{cases}
      P_{\mathrm{ok}} & (\text{一致の場合}) \\
      P_{\mathrm{sub}} & (\text{不一致の場合})
    \end{cases}
  \end{align}
  ...
  \begin{align}
    \Delta P = \begin{cases}
      P_{\mathrm{ins}} + P_{\mathrm{ok}} & (\text{一致の場合}) \\
      P_{\mathrm{ins}} + P_{\mathrm{sub}} & (\text{不一致の場合})
    \end{cases}
  \end{align}
\end{itemize}
...
機械学習モデルによって行われる．本研究で使用するベースコーラーの
アーキテクチャBonitoの概要を図\ref{fig:bonito}に示す\cite{bonito}．
...
\chapter{ナノポアシーケンサーによる読み出しを前提とした誤り訂正手法}\label{proposed_method}
...
を$\bm{P}_{\mathrm{STATS}}$とおき，後述の手法により報酬とペナルティの算出に利用する．
...
% した場合のベースコール結果に相当する．
$\bm{S}'$の各要素に対して自然対数をとって$-1$を乗じた行列を$\bm{P}_{\mathrm{CTC}}$とおく．
$\bm{P}_{\mathrm{CTC}}$の各列の要素は，各時点において各塩基または空白が出力される
確率に基づくスコアを表す．
...
$\bm{P}_{\mathrm{CTC}}$から取得し，これを用いて報酬やペナルティを決定する．

\subsection{報酬とペナルティの最適化} \label{reward_penalty_optimization}
既存手法では，報酬$P_{\mathrm{ok}}$と置換・挿入・消失エラーに対する
ペナルティ$P_{\mathrm{sub}}$，$P_{\mathrm{ins}}$，$P_{\mathrm{del}}$は一定値を用いるが，本研究では
\ref{stat_model}節および\ref{ctc_output}節で述べたナノポアシーケンサーの読み出し特性と
ベースコーラーのCTC出力を考慮した新しい報酬$P'_{\mathrm{ok}}$と
ペナルティ$P'_{\mathrm{sub}}$，$P'_{\mathrm{ins}}$，$P'_{\mathrm{del}}$を提案し，以下のように定義する．
\begin{equation}
  \begin{split}
    P'_{\mathrm{ok}} = P_{\mathrm{ok}} &+ \alpha_{\mathrm{ok}}(\bm{P}_{\mathrm{STATS}}(x_{hypo}, x_{max}(i))-\mu_{\mathrm{STATS}\_\mathrm{ok}}) \label{opt1} \\ 
     &+ \beta_{\mathrm{ok}}(\bm{P}_{\mathrm{CTC}}(i, x_{hypo})-\mu_{\mathrm{CTC}\_\mathrm{ok}}) 
  \end{split}
\end{equation}
\begin{equation}
  \begin{split}
    P'_{\mathrm{sub}} = P_{\mathrm{sub}} &+ \alpha_{\mathrm{sub}}(\bm{P}_{\mathrm{STATS}}(x_{hypo}, x_{max}(i))-\mu_{\mathrm{STATS}\_\mathrm{sub}}) \label{opt2} \\
    &+ \beta_{\mathrm{sub}}(\bm{P}_{\mathrm{CTC}}(i, x_{hypo})-\mu_{\mathrm{CTC}\_\mathrm{sub}})
  \end{split}
\end{equation}
\begin{equation}
  \begin{split}
    P'_{\mathrm{ins}} = P_{\mathrm{ins}} &+ \alpha_{\mathrm{ins}}(\bm{P}_{\mathrm{STATS}}(N, x_{max}(i-1))-\mu_{\mathrm{STATS}\_\mathrm{ins}}) \label{opt3}\\
    &+ \beta_{\mathrm{ins}}(\bm{P}_{\mathrm{CTC}}(i-1, x_{hypo})-\mu_{\mathrm{CTC}\_\mathrm{ins}})
  \end{split}
\end{equation}
\begin{equation}
  \begin{split}
    P'_{\mathrm{del}} = P_{\mathrm{del}} &+ \alpha_{\mathrm{del}}(\bm{P}_{\mathrm{STATS}}(x_{hypo}, N)-\mu_{\mathrm{STATS}\_\mathrm{del}}) \label{opt4}
  \end{split}
\end{equation}
ここで，$x_{hypo}$は仮説のビット列から出力される塩基を表し，$i$は仮説が参照する
$\bm{P}_{\mathrm{CTC}}$の行番号を表す．また，$x_{max}(i)$は$\bm{P}_{\mathrm{CTC}}$の$i$行目において
出力確率が最大となる塩基を表す．$\bm{P}_{\mathrm{STATS}}(X,Y)\quad(x,y\in \{N,A,C,G,T\})$は$\bm{P}_{\mathrm{STATS}}$における
ラベル$x$からラベル$y$への遷移に対応するスコアを表す．
$\bm{P}_{\mathrm{CTC}}(i,X)$は，$\bm{P}_{\mathrm{CTC}}$の$i$行目におけるラベル$X$のスコアを表す．
...
挿入エラー，消失エラーが発生したときの$\bm{P}_{\mathrm{STATS}}$のスコアの期待値を表し，
% すなわち
% \begin{align}
...
報酬と各ペナルティは，固定の値$P_{\mathrm{ok}}$，$P_{\mathrm{sub}}$，$P_{\mathrm{ins}}$，$P_{\mathrm{del}}$に対して，
期待値が0になるように正規化された補正項を加算することで計算される．
各補正項には，調整用のパラメータ$\alpha_{\mathrm{ok}}$，$\alpha_{\mathrm{sub}}$，$\alpha_{\mathrm{ins}}$，$\alpha_{\mathrm{del}}$，
$\beta_{\mathrm{ok}}$，$\beta_{\mathrm{sub}}$，$\beta_{\mathrm{ins}}$が乗じられており，
...
\begin{enumerate}
  ...
  \begin{align}
    S_{match}(k) = \sum_{j=0}^{l_{sep}-1} \bm{P}_{\mathrm{CTC}}[k+j, S[j]] \label{eq:separator_detection}
  \end{align}
...
\chapter{シミュレーション評価} \label{simulation}
...
得られたDNA鎖に対し，
ナノポアシーケンサーによる電流波形をSquigulator\cite{squigulator}と呼ばれる
シミュレータを用いて得た．
...
符号化率
$0.75,0.60,0.50,0.33$で行い，塩基長は400\,ntとした．
...
\begin{table}
  ...
  \end{tabular}
\end{table}
提案手法を用いることで，各符号化率において
既存手法と比べてヒープ探索数を削減し，復号失敗率を低減することができた．
ナノポアシーケンサーの読み出し特性に基づく補正項のみを適用した場合，
ヒープ探索数が43\%から61\%削減され，ビット消失率は60\%から77\%低減した．
CTC出力に基づく補正項のみを適用した場合，
...
これをDNA鎖1本の塩基長を500\,ntから15000\,ntの範囲で変化させながら行い，
...
提案手法におけるセグメント化では，セグメント長は400\,ntとした．
復号におけるヒープ探索では，既存手法では塩基長に関わらずヒープサイズの上限を
1000000とした場合と，塩基長が400\,ntのときのヒープサイズ上限を1000000とし，
